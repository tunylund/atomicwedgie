const quotes = [
  ...
  export function wedgied(enemy: string) {
    return `You got ${randomFrom(wedgies)}  by ${enemy}!`;
  }
  export function banzaid(enemy: string) {
    if(Math.random() > 0.85) {
      return `${enemy} opened a can of whoopass on you!`;
    } else {
      return `You got ${randomFrom(banzais)} by ${enemy}!`;
    }
  }
