...
class Player extends enchant.Sprite {
  ...
  constructor (color) {
    ...
    this.image = game.assets[resources["man" + this._suffix]]
  ...
  move () {
    ...
    if (this.v) {
      ...
      if(game.frame % Math.ceil(this.walksps/this.speedMultiplier) == 0)
        game.assets[resources["walk" + Math.ceil(Math.random()*3)]].play(true)
  ...
  updateFrameSet () {
    ...
    if(this.v) {
      ...
      if(this.banzaid) {
        ...
        img = game.assets[resources["man" + this._suffix + "BanzaiWalk"]]
  ...
  performWedgie () {
    ...
    game.assets[resources["performWedgie" + Math.ceil(Math.random()*4)]].play()
  }
  ...
  performBanzaiHit () {
    ...
    if(!this.banzaiSoundPlayed) {
      game.assets[resources["performBanzai" + Math.ceil(Math.random()*3)]].play()
  ...
  toggleBanzaiMode () {
    ...
    if(this.banzaiMode) {
      ...
      game.assets[resources["banzaiScream" + Math.ceil(Math.random()*3)]].play()
    } else {
      this.walkSpeed = walkSpeed
    }
    this.shouldUpdate = true
  ...
  wedgie () {
    ...
    this.performingWedgie = false
    this.shouldUpdate = true
    game.assets[resources["arrgh" + Math.ceil(Math.random()*4)]].play()
  ...
  consumePill (pill) {
    ...
    if(pill.type == "green" && Math.random() > 0.75)
      game.assets[resources.uliuliuli].play()
    else {
      let r = Math.ceil(Math.random()*3)
      game.assets[resources["pill" + r]].play()
    }
  }
